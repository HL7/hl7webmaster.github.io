<template>
  ...
  >
    ...
    <div
      v-once
      id="hubspotForm"
      class="bg-white px-8 py-2 rounded-lg w-full md:w-1/2 mt-6 md:mt-0 xl:w-1/3"
    ></div>
  </div>
</template>
<script>
export default {
  ...
  mounted() {
    ...
    contactFormScript.src = 'https://js.hsforms.net/forms/v2.js'
    document.body.appendChild(contactFormScript)
    contactFormScript.addEventListener('load', () => {
      if (window.hbspt) {
        window.hbspt.forms.create({
          region: 'na1',
          portalId: '8047841',
          formId: '9f72a8ab-5458-49c6-a88b-f27688d23792',
          target: '#hubspotForm',
        })
      }
    })
